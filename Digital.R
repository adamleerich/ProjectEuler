...
as.Digital.Digital <- function(d) {
  ...
  while(i < length(d)) {
    ...
    if(d[i] >= base) {
      ...
      d[i] <- 0
      d[i:(i + len.n - 1)] <- d[i:(i + len.n - 1)] + unclass(n)
    } 
  }
...
as.Digital.integer <- function(num, base = 10) {
  ...
  elements <- as.integer(log(my.num) / log(base) + 2)
...
`+.Digital` <- function(x, y) {
  ...
  if(!(x.digital & y.digital)) {
    stop('+.Digital does not support scalars at this time')
  }
  ...
  x.base <- attr(x, 'base')
  y.base <- attr(y, 'base')
  out.base <- max(x.base, y.base)
  
  if(x.base != y.base) {
    stop('Bases must be the same in Digital addition')
  }
  
  x.len <- length(x)
  y.len <- length(y)
  out.len <- max(x.len, y.len)
  
  
  out <- new_Digital(0, base = out.base, length = out.len)
  
  
  out[1:out.len] <- c(unclass(x), rep(0, out.len - x.len)) +
                    c(unclass(y), rep(0, out.len - y.len))
